/* tslint:disable */
/* eslint-disable */
export const memory: WebAssembly.Memory;

export function return_details(a: number, b: number): void;
export function print_string(a: number, b: number): void;
export function print_str(a: number, b: number): void;
export function print_int(a: number): void;
export function print_long(a: number): void;
export function print_short(a: number): void;
export function print_float(a: number): void;
export function print_double(a: number): void;

export function on_enable(): void;

export function on_disable(): void;

export function register_plugin(): void;
export function __wbindgen_malloc(a: number, b: number): number;
export function __wbindgen_realloc(a: number, b: number, c: number, d: number): number;
